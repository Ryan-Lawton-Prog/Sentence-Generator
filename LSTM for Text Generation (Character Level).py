...
with tf.Session() as sess:
    ...
    for e in range(num_epochs): # num_epochs is 5 for test, but should be higher
        ...
        with tf.variable_scope("rnn", reuse=True):
            ...
            saver.save(sess, 'sentence-generator', global_step=e)
    with tf.variable_scope("rnn", reuse=True):
            sample_model = LSTMModel(sample=True)
            inp = "The "
            while inp != "":
                print (sample_model.sample(sess, data_loader.chars , data_loader.vocab, num=50, prime=inp, sampling_type=1))
                print ('----------------------------------')
                inp = input("Sentence primer: ")
